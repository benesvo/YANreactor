...
float pump_coef = 0.00020354;   //koeficient pro prepocet output >> Vfeed ([ms] >> [ml])
...
void loop()
{
  ...
  Serial.print(float(millis() / 3600000.0), 5); Serial.print(";");   //cas [h]
  Serial.print(hmotnost_akt, 5); Serial.print(";");                  // [g]
  ...
  cas_predch = millis();    //aktualizuje cas posledni probehle loop
